...
\begin{enumerate}
\item \textbf{Block 1 --- Power allocation (RL):} The RL agent selects an action from $\mathcal{A}$ ($|\mathcal{A}|=42$), the Cartesian product of 7 total-power fractions $P_{\mathrm{tot}}/P_{\max} \in \{0.20, 0.35, 0.50, 0.65, 0.80, 0.90, 1.00\}$ and 6 per-user allocation modes: equal, channel-proportional, inverse-channel, SINR-deficit ($\propto [\gamma_{\min} - \text{SINR}_k]^+$), water-filling, and max-protection ($\propto 1/\max(\text{SINR}_k,\epsilon)$, worst-user focus).
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\columnwidth]{ieee_fig9_evaluation.pdf}
\caption{Evaluation comparison: (a)~system rate and (b)~SINR protection (mean $\pm$ std, 5 seeds); Fuzzy WoLF-PHC is highest with the smallest variance.}
\label{fig:eval_bar}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\columnwidth]{ieee_fig5_vs_pmax.pdf}
\caption{(a)~System rate and (b)~SINR protection vs.\ maximum transmit power $P_{\max}$; WoLF-PHC scales most effectively with power.}
\label{fig:pmax}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\columnwidth]{ieee_fig6_vs_nris.pdf}
\caption{(a)~System rate and (b)~SINR protection vs.\ IRS array size $N_\mathrm{IRS}$; gains grow but saturate near $N_\mathrm{IRS}=256$.}
\label{fig:nris}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\columnwidth]{ieee_fig7_vs_sinr.pdf}
\caption{(a)~System rate and (b)~SINR protection vs.\ UE SINR target $\gamma_{\min}$; WoLF-PHC degrades most gracefully.}
\label{fig:sinr}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=0.75\columnwidth]{ieee_fig12_vs_pjammer.pdf}
\caption{(a)~System rate and (b)~SINR protection vs.\ maximum jammer power $P_J$; a regime transition occurs near $10$--$15$\,dBm.}
\label{fig:pjammer}
\end{figure}
